...
class AudioManager {
	...
	enum Sound: String, CaseIterable, Identifiable {
		case actionSynth = "action synth"
		case funnyGuitar = "funny guitar"
		case slowFunky = "slow funky techno sounds"
		var id: Self {self}
	}
	
	func playSound(named fileName: String) {
		guard let url = Bundle.main.url(forResource: fileName, withExtension: "mp3") else {
			print("Error: Could not find \(fileName).mp3 in the project bundle.")
			return
		}
...
struct HomeScreen: View {
	...
	var body: some View {
		...
		.alert("You did it!", isPresented: $isPresentingCongrats, actions: {
		...
		.onChange(of: timerRunning) { old, new in
			handleTimerRunningChange(old: old, new: new)
		}
	...
	/// Helper to handle timerRunning change to avoid heavy type inference in onChange closure
	func handleTimerRunningChange(old: Bool, new: Bool) {
		print("timerRunning changed to \(new)")
		UIApplication.shared.isIdleTimerDisabled = new
	}
	
	/// Helper used by the warning alert OK button
	func confirmHintAndStart() {
		hintShown = true
		startTimer()
	}
	...
	@AppStorage("sound_name") var soundName = AudioManager.Sound.actionSynth
...
#if DEBUG
	@AppStorage("timer_setting") var timerSetting = TimeInterval(2)
#else
	...
	func loadLocalFile() {
		...
		guard let fileURL = Bundle.main.url(forResource: "data", withExtension: "json") else {
			print("Error: File not found in the main bundle.")
			return
		}
		...
		do {
			...
			// Process your fileData here (e.g., JSONDecoder)
			print("Successfully loaded \(fileData.count) bytes.")
		} catch {
	...
	func timerDidExpire() {
		print("Timer O Din Din Done")
		AudioManager.shared.playSound(named: soundName.rawValue)
